...
Investigate each principal's flaws in the model, not as labels: a costly habit, mistaken belief, avoidance, conflicting priorities or a strength overused in the wrong setting, each a process over the life with the event that taught it (often a strategy that once served a deep want), the situations in which it takes over, where the same trait is a strength and where it does harm, what it costs in the story's choices, and whether the person sees it. Ground this in numerical distinctions and life developments, preserve competence, and do not equate hardship or low sampled values with a flaw. Record the review and any local repair in the graph.
