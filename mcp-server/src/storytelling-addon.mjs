...
import { trajectoryInstructions } from './storytelling-trajectories.mjs';
import { modelDepthPrepareSchema, modelDepthRecordSchema, modelDepthInstructions, prepareModelDepthReview, recordModelDepthReview, readModelDepthReview } from './storytelling-depth.mjs';
...
export function registerStorytellingAddon(server, service) {
  ...
  for (const [name, method, schema, description, readOnly, idempotent = true] of [
    ...
    server.registerTool(name, { description: `${storyScopeInstructions}\n\n${name.startsWith('life_story_scene_') ? `${passageInstructions}\n\n` : ''}${description}`, inputSchema: schema,
