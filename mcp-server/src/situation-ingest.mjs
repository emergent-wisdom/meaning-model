...
export async function ingestSituation(raw, estimator, service) {
  ...
  const supplied = new Map(input.distributions.map((distribution) => [`${distribution.questionId}|${distribution.eventId}`, distribution]));
  ...
  for (const question of input.questions) {
    ...
    for (const request of buildCutShareQuestions(shaped, targets)) {
      ...
      const distribution = supplied.get(`${question.id}|${target.id}`);
      ...
      proposals.push({ questionId: question.id, eventId: target.id, ...proposalFromProbabilities(shaped, target, answer.probabilities, { label: `${estimator.backend}:${model}`, confidence: Number(answer.confidence) }) });
    }
  }
  ...
  if (!input.apply) return { ...common, applied: null, worldMutation: false, graphMutation: false, nextStep: 'Review the events and proposals, then call again with apply to register them as one model revision.' };
  ...
  let rebound = null; let notes = null;
  if (input.graph) {
    let graphHash = input.graph.graphHash;
    if (input.graph.rebind) { rebound = await rebindNarrativeGraph(service, { requestId: `${input.requestId}-rebind`, graphHash, modelHash: revised.modelHash, accessScopes: input.graph.accessScopes, reason: `Rebind after situation ingest ${input.requestId}.` }); graphHash = rebound.graphHash; }
    if (input.graph.notes.length) notes = await recordIngestNotes(service, { graphHash, accessScopes: input.graph.accessScopes, requestId: `${input.requestId}-notes`, notes: input.graph.notes, eventIds: new Set(events.keys()), provenance });
  }
...
export async function recordIngestNotes(service, { graphHash, accessScopes, requestId, notes, eventIds, provenance }) {
  ...
  if (view.graph_hash !== graphHash) throw new Error('Notes must be recorded against the exact graph revision.');
  ...
  for (const [index, note] of notes.entries()) {
    if (view.nodes.some((node) => node.id === note.nodeId)) throw new Error(`Note ${note.nodeId} already exists; use a new node ID.`);
  ...
  const stored = await service.applyNarrativeBatch({ requestId, previousGraphHash: graphHash, narrativeBatch: { schema: 'life-sim-rust-narrative-batch/v1', previous_graph_hash: graphHash, reason: `Record ${notes.length} understanding note${notes.length === 1 ? '' : 's'} from situation ingest.`, provenance, add_roots: addRoots, add_nodes: nodes, add_edges: edges } });
