...
export async function rebindNarrativeGraph(service, raw) {
  ...
  return { ...stored, schema: 'meaning-model-narrative-rebind/v1', previousGraphHash: input.graphHash, previousModelHash, modelHash: input.modelHash, lineageSteps, droppedModelAnchorEdgeIds, retainedNodeCount: successor.nodes.length, historicalAssessmentsRetained: true, worldMutation: false, nextStep: droppedModelAnchorEdgeIds.length ? 'Historical depth and review nodes remain but their predecessor-model anchors were removed; record fresh assessments against the successor model before committing further prose.' : 'The graph now reads the successor model; no model anchors were dropped. Ask the model\'s questions of it (life_model_questions) before committing further prose.' };
}
