...
Choose the smallest useful changes. Distinguish missing or inadequate model causes from a sound model that the prose fails to express, and from purposeful ambiguity or withheld information with a modeled disclosure process. Model refinement, local trajectory repair, narrative disclosure and prose revision are different operations; record which is needed and why. Preserve what works. Deepening can clarify, compress, remove or reorganize as well as add; more words, larger numbers, more shocks or more visible traits do not establish improvement. Keeping a passage unchanged is valid. Do not blindly reroll, broaden numerical bounds to excuse an inconvenient result, or revise an earlier intention solely to declare success.
