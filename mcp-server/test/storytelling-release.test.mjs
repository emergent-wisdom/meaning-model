...
test('text the author cut from a split scene never reaches readers', async (t) => {
  ...
  await assert.rejects(release(f.service, cut.graphHash), /Containers scene\.1 hold text the render does not show/);
  const released = await release(f.service, cut.graphHash, { clearHiddenText: true });
...
test('a release never narrows prose that is already public, and reaches scenes inside chapters', async (t) => {
  ...
  const released = await release(f.service, f.graphHash);
